...
class AbstractClassBuilder {
    ...
    static public function build():Array<Field> {
        ...
        for (f in fields) {
            if (f.access.has(AStatic)) continue; // We only care for non-static methods

            switch (f.kind) {
                case FFun(fun):
                    if (fun.expr == null) { // Method without implementation
                        absFields.set(f.name, true);

                        var funRetCheck = switch (fun.ret) {
                            case TPath(p): p.pack.length == 0 && p.name == "Void";
                            default: false;
                        }

                        //give a method body so it compiles
                        fun.expr = if (fun.ret == null || funRetCheck) {
